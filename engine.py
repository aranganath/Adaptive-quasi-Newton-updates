...
class Engine(object):
    ...
    def _train_one_epoch(self, epoch, step, next_checkpoint_step):
        ...
        for i, batch in enumerate(dataloader):
            ...
            # if self.configs["model"]["name"] == "combative":
            #     keys = batch[0].keys()
            #     inputs = [tuple([self.to_device(x[item]) for item in keys]) for x in batch]
            ...
            # self.writer.add_scalar('Loss/Train', loss_val, epoch)
            if loss.isnan():
